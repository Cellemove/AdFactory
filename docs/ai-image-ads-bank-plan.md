...
The default image renderer is now `gemini-3.1-flash-image` (Nano Banana 2) on Vertex at 2K. Standard global image-output pricing is approximately $0.101 per image: $2.02 for 20 images or $2.525 for 25, before reference analysis, concept planning, input/reasoning tokens, and retries. The 50-second progress estimate remains conservative until measured on this workload. Logo compositing is local and adds no generation API charge. `IMAGE_MODEL`, `IMAGE_SIZE`, `IMAGE_COST_USD` and `IMAGE_SECONDS` override the defaults; size-specific output pricing follows `IMAGE_SIZE` unless an explicit cost override is supplied. Source: [Google Cloud pricing](https://cloud.google.com/vertex-ai/generative-ai/pricing).

Earlier live verification used `gemini-3-pro-image` (Nano Banana Pro) at 2K: 1856 x 2304 for 4:5 in 40-60 seconds, with clean display text and aspect-ratio adherence for 4:5, 1:1 and 9:16. `gemini-2.5-pro` returned all 25 concepts in one JSON response in ~72 seconds. Those measurements describe the previous renderer; Flash 3.1 output quality and latency have not yet been benchmarked here.
