...
export interface RecentExcavation {
  id: string;
  avatarName: string;
  subCount: number;
  createdAt: string;
...
// G1 Avatar Excavation, embedded in the pipeline index. Collapsed by default;
// opens automatically when a past excavation is deep-linked (?excavation=id).
export function ExcavateSection({
  initial,
  recent,
}: {
  ...
  return (
    <details className="card overflow-hidden border-brand-pink/40 bg-brand-pink/5 p-0" open={initial != null}>
      <summary className="flex cursor-pointer flex-wrap items-center justify-between gap-3 p-4 [&::-webkit-details-marker]:hidden">
        <div className="min-w-0">
          <div className="text-[11px] font-semibold uppercase tracking-wide text-brand-plum">Start here · G1</div>
          <div className="text-sm font-semibold text-ink-900">Excavate a new avatar</div>
          <p className="mt-0.5 text-xs text-ink-500">
            One surface desire + one problem. We scrape real verbatims across Reddit, YouTube, Quora &amp;
            reviews, then map the avatar and every sub-avatar — pick one to deep-dive.
          </p>
        </div>
        <span className="btn shrink-0 text-xs">Open ▾</span>
      </summary>

      <div className="space-y-4 border-t border-brand-pink/30 p-4">
        {/* Seed inputs */}
        ...
        {result && (
          <section className="space-y-4">
            <div className="card border-brand-pink/40 bg-white">
