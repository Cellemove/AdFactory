...
type BatchResult = { variantId?: string; variantName?: string; frameworkId?: string; frameworkName?: string; projectId: string | null; error: string | null };
...
export default async function ScriptBatchPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <div className="space-y-6">
    <header><Link href="/scripts" className="text-sm text-ink-500 hover:underline">← Script Studio</Link><h1 className="mt-2 text-2xl font-semibold">Draft comparison</h1><p className="mt-1 text-sm text-ink-500">One idea, compared across {results.length} selected variants. Each draft remains independently editable.</p></header>
    <div className="grid gap-4 lg:grid-cols-2">
      {results.map((result) => {
        const project = result.projectId ? projects.get(result.projectId) : null;
        const variantId = result.variantId ?? result.frameworkId ?? "unknown";
        const variantName = result.variantName ?? result.frameworkName ?? "Variant";
        if (!project) return <article key={variantId} className="card border-red-200 bg-red-50"><h2 className="font-semibold">{variantName}</h2><p className="mt-2 text-sm text-red-700">{result.error || "This draft was not created."}</p></article>;
