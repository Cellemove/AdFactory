...
import { generateScriptProjectDraft, saveScriptDocument, snapshotScriptProject } from "@/app/actions/scripts";
...
export function ScriptStudioClient({ projectId, initialDocument, initialRevision, initialVersion }: { projectId: string; initialDocument: ScriptDocument; initialRevision: number; initialVersion: number }) {
  ...
  return (
    <div className="space-y-4">
      <div className="sticky top-20 z-20 flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-ink-200/70 bg-white/90 p-3 shadow-card backdrop-blur-xl">
        <div className="flex items-center gap-2"><div className="rounded-full bg-ink-100 p-1"><button className={`rounded-full px-3 py-1 text-sm ${view === "modules" ? "bg-white shadow-sm" : "text-ink-500"}`} onClick={() => setView("modules")}>Modules</button><button className={`rounded-full px-3 py-1 text-sm ${view === "document" ? "bg-white shadow-sm" : "text-ink-500"}`} onClick={() => setView("document")}>Document</button></div><span className="text-xs text-ink-500">{totalDuration}s / {document.targetDurationSec}s · {issues.length} checks</span></div>
        <div className="flex flex-wrap gap-2"><button className="btn" onClick={generateDraft} disabled={pending}>{pending ? "Generating…" : "AI fill all"}</button><button className="btn" onClick={downloadScript}>Download script</button><button className="btn" onClick={snapshot} disabled={pending}>Create version</button><button className="btn btn-primary" onClick={save} disabled={pending}>{pending ? "Working…" : "Save changes"}</button></div>
      </div>
        ...
        <div className="space-y-3">
          {document.modules.map((module, index) => {
            ...
            return <section key={module.id} className={`card space-y-3 ${module.locked ? "bg-ink-50" : ""}`}>
              <div className="flex flex-wrap items-center justify-between gap-2"><div className="flex items-center gap-2"><span className="flex h-7 w-7 items-center justify-center rounded-full bg-ink-900 text-xs text-white">{index + 1}</span><input aria-label="Beat label" className="input max-w-xs font-semibold" value={module.label} disabled={module.locked} onChange={(event) => updateModule(module.id, { label: event.target.value })} /><span className="tag">{module.kind}</span></div><div className="flex gap-1"><button className="btn btn-ghost px-2" onClick={() => move(index, -1)} disabled={index === 0}>↑</button><button className="btn btn-ghost px-2" onClick={() => move(index, 1)} disabled={index === document.modules.length - 1}>↓</button><button className="btn btn-ghost" onClick={() => updateModule(module.id, { locked: !module.locked })}>{module.locked ? "Unlock" : "Lock"}</button><button className="btn btn-ghost text-red-700" onClick={() => removeModule(module.id)}>Remove</button></div></div>
              <div className="grid grid-cols-1 gap-3 lg:grid-cols-[1fr_1fr_8rem]"><div><label className="label">Spoken copy</label><AutoResizeTextarea className="input min-h-11" value={module.spokenText} disabled={module.locked} onChange={(event) => updateModule(module.id, { spokenText: event.target.value })} /></div><div><label className="label">Visual direction</label><AutoResizeTextarea className="input min-h-11" value={module.visualDirection} disabled={module.locked} onChange={(event) => updateModule(module.id, { visualDirection: event.target.value })} /></div><div><label className="label">Seconds</label><input className="input" type="number" min={0} max={600} value={module.durationSec} disabled={module.locked} onChange={(event) => updateModule(module.id, { durationSec: Number(event.target.value) })} /></div></div>
              <div><label className="label">On-screen text</label><input className="input" value={module.onScreenText} disabled={module.locked} onChange={(event) => updateModule(module.id, { onScreenText: event.target.value })} /></div>
          ...
          <button className="btn w-full border-dashed" onClick={addModule}>+ Add beat</button>
        </div>
      ) : (
