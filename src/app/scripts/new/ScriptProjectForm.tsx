  ...
  return (
    <div className="card space-y-5">
      <div className="grid-fields">
        ...
        <div><label className="label">Teardown2 source</label><select className="input" disabled={!props.teardownConfigured || props.teardowns.length === 0} value={form.teardownRecordId} onChange={(event) => setForm({ ...form, teardownRecordId: event.target.value })}><option value="">No teardown source</option>{props.teardowns.map((item) => <option key={item.id} value={item.id}>{item.name}</option>)}</select><p className="mt-1 text-xs text-ink-500">{props.teardownWarning ? `Unavailable: ${props.teardownWarning}` : !props.teardownConfigured ? "Set TEARDOWN_API_BASE_URL and TEARDOWN_INTERNAL_TOKEN to enable imports." : `${props.teardowns.length} completed records available.`}</p></div>
