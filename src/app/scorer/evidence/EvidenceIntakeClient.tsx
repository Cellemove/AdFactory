    ...
    <div className="space-y-6">
      <form className="card space-y-5" onSubmit={submit}>
        ...
        <div className="grid gap-4 md:grid-cols-2">
          ...
          <label className="md:col-span-2"><span className="label">Performance evidence</span><textarea className="input min-h-20" value={form.performanceEvidence ?? ""} onChange={(event) => set("performanceEvidence", event.target.value)} placeholder="Optional — for example: ROAS 2.4, $18k spend, source report and date." />{fieldError("performanceEvidence")}</label>
        </div>
