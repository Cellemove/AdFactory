 ...
 */
async function loadPlaybookAds(brand: string, taxonomyVersion: string, mode: ResearchMode): Promise<Array<PlaybookAd & { extractRunId: string }>> {
  ...
  const runs = await supabase.from("CorpusExtractRun").select("*").in("competitorAdId", adIds).eq("taxonomyVersion", taxonomyVersion).eq("researchMode", mode).eq("status", "complete").order("createdAt", { ascending: false });
  ...
  const [beats, segments, media, transcripts] = await Promise.all([
...
export async function buildAndSavePlaybook(brand: string, options: { taxonomyVersion?: string; mode?: ResearchMode } = {}): Promise<SavePlaybookResult> {
  const mode = options.mode ?? defaultResearchMode();
  const engineVersion = modeVersion(PLAYBOOK_ENGINE_VERSION, mode);
  const taxonomyVersion = options.taxonomyVersion ?? CORPUS_TAXONOMY_VERSION;
  const ads = await loadPlaybookAds(brand, taxonomyVersion, mode);
  ...
  const reachable = (state.data ?? []).filter((row) => mode === "speech_only" || !MEDIA_FAILED.has(row.mediaStatus ?? "")).length;
  ...
  playbook.engineVersion = engineVersion;
  playbook.researchMode = mode;
  if (mode === "speech_only") playbook.copy.tone = playbook.copy.tone.map((stat) => stat.key === "caption" ? { ...stat, value: "Unassessed", note: "Speech-only evidence" } : stat);
  if (mode === "speech_only") playbook.caveats = playbook.caveats.filter((c) => !c.includes("before visual direction was captured"));
  if (mode === "speech_only") playbook.caveats.unshift("Speech-only playbook: sequences describe spoken copy. Visuals, production formats and on-screen text are unassessed.");
  ...
  const existing = await supabase.from("CorpusBrandPlaybook").select("id")
    .eq("brand", brand).eq("taxonomyVersion", taxonomyVersion).eq("engineVersion", engineVersion).eq("inputHash", inputHash).maybeSingle();
  ...
  const write = await supabase.from("CorpusBrandPlaybook").insert({
    id: playbookId([brand, taxonomyVersion, engineVersion, inputHash]),
    brand,
    taxonomyVersion,
    engineVersion,
    adCount: ads.length,
...
export async function latestBrandPlaybook(brand: string, taxonomyVersion: string = CORPUS_TAXONOMY_VERSION, mode: ResearchMode = defaultResearchMode()): Promise<LatestPlaybook | null> {
  const engineVersion = modeVersion(PLAYBOOK_ENGINE_VERSION, mode);
  const result = await supabase.from("CorpusBrandPlaybook").select("*")
    .eq("brand", brand).eq("taxonomyVersion", taxonomyVersion).eq("engineVersion", engineVersion)
...
export async function listPlaybookBrands(taxonomyVersion: string = CORPUS_TAXONOMY_VERSION, mode: ResearchMode = defaultResearchMode()): Promise<Array<{ brand: string; adCount: number; createdAt: string }>> {
  const engineVersion = modeVersion(PLAYBOOK_ENGINE_VERSION, mode);
